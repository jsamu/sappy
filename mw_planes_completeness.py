...
def loop_hal(sat, mask_key, exec_func, redshift_list_file_path, **kwargs):
    '''
    ...
    if sat.sat_type == 'hal':
        for hal_name in sat.hal_name:
            for z_index, z in enumerate(sat.redshift):
                ...
                redshift_list = pd.read_csv(redshift_list_file_path, sep=' ')
                z_index_600 = np.where(redshift_list == z)[0][0]

                loop_dict[hal_name].append(
                    exec_func(hal=hal, hal_mask=hal_mask, host_name=hal_name, 
                    redshift_index=z_index_600, **kwargs))
            ...
            for host_name, host_str in zip(sat.hal_name[pair_name], ['host.', 'host2.']):
                for z_index, z in enumerate(sat.redshift):
                    ...
                    loop_dict[host_name].append(
                        ...
                        host_name=host_name, redshift_index=z_index_600, **kwargs))
    else:
...
def rand_rms_min(
    ...
    # apply disk mask
    sat_coords = select_out_of_disk(sat_coords, host_axes_dict, host_name, redshift_index)
...
def axis_ratio(
    ...
    '''
    sat_coords = hal.prop(host_str+'distance')[hal_mask]
    sat_masked_coords = select_out_of_disk(sat_coords, host_axes_dict, host_name, redshift_index)
...
def orbital_ang_momentum(
    ...
    position_masked, disk_mask = select_out_of_disk(sat_coords, host_axes_dict, host_name, redshift_index, return_mask=True)
...
def orbital_pole_dispersion(
    ...
    if np.sum(hal_mask) == 0:
        ...
        j_vec = orbital_ang_momentum(hal, hal_mask, host_str, host_name, 
            redshift_index, norm=True, host_axes_dict=host_axes_dict)
