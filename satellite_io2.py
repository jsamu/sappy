...
def load_hals(
    ...
    for directory, name in zip(sim_directories, sim_names):
        if len(snapshot_index) == 1 or type(snapshot_index) is int:
            ...
            # add in baryon fraction-corrected vel.circ.max for DMO case
            if baryon_frac is True:
                for snap_dict in snap_list:
                    #fb_correction = np.sqrt(1 - snap_dict.Cosmology['baryon.fraction'])
                    #snap_dict['vel.circ.max.fbcorr'] = snap_dict['vel.circ.max']*fb_correction
            ...
            #first_snap = next((snap for snap in snap_list if snap), snap_list[600])
            #snap_indices = first_snap.Snapshot.get_snapshot_indices('redshift', np.array(snapshot_redshifts))
...
class MaskHalo(object):
    def __init__(self):
        pass

    def multiple_catalog_masks(self, sat, mask_names, dmo=False):
        catalog_mask_dict = {}
        for mask in mask_names:
            catalog_mask_dict[mask] = LoopHalo.loop_sat(sat, self.mask_hosts, **{'mask_key':mask, 'dmo':dmo})
        return catalog_mask_dict

    def mask_hosts(self, sat, hal_name, redshift_index, mask_key, dmo=False):
        '''
        ...
        if dmo == False:
            snap_mask = self.mask_hal_baryonic(hal, sat, mask_keys=mask_key)

        elif dmo == True:
            snap_mask = self.mask_hal_dmo(hal, sat, hal_name, redshift_index, mask_keys=mask_key)

        return snap_mask

    def mask_catalogs(self, sat, mask_keys, dmo=False):
        '''
        Generate a mask for a single snapshot adhering to conditions given in mask_cuts_*.
        '''
        sat_mask_dict = {}
        for host_key in sat.hal_catalog.keys():
            ...
            for redshift_index in range(len(sat.redshift)):
                ...
                if dmo is True:
                    snap_mask = self.mask_hal_dmo(hal=hal, sat=sat, hal_name=host_key, redshift_index=redshift_index, mask_keys=mask_keys)
                else:
                    snap_mask = self.mask_hal_baryonic(hal=hal, sat=sat, mask_keys=mask_keys)
                host_masks.append(snap_mask)
            sat_mask_dict[host_key] = host_masks

        return sat_mask_dict

    def mask_hosts_lite(self, sat_params, sat_data, mask_keys, dmo=False):
        '''
        Generate a mask for a single snapshot adhering to conditions given in mask_cuts_*.
        '''
        sat_mask_dict = {}
        for host_key in sat_data.keys():
            ...
            for redshift_index in range(len(sat_params.redshift)):
                ...
                if dmo is True:
                    snap_mask = self.mask_hal_dmo(hal=hal, sat=sat_params, hal_name=host_key, redshift_index=redshift_index, mask_keys=mask_keys)
                else:
                    snap_mask = self.mask_hal_baryonic(hal=hal, sat=sat_params, mask_keys=mask_keys)
                host_masks.append(snap_mask)
            sat_mask_dict[host_key] = host_masks

        return sat_mask_dict

    def mask_hal_baryonic(self, hal, sat, mask_keys):
        '''
    ...
    def mask_hal_dmo(self, hal, sat, hal_name, redshift_index, mask_keys):
        '''
    ...
    def mask_lg_baryon_cat(self, sat):
        """
        ...
        for pair_name in sat.hal_catalog.keys():
            ...
            host_list = ['host', 'host2']
            for host_name, host_str in zip(sat.hal_name[pair_name], host_list):
                for hal in sat.hal_catalog[pair_name]:
                    # choose only satellites of a single host galaxy
                    host_ind = np.unique(hal[host_str+'.index'])[0]
                    host_system_mask = hal[host_str+'.index'] == host_ind
    ...
    def reset_sm_limits(self, sat, lower_lim, upper_lim=1e10, obs=False):
        sat.star_mass = [lower_lim, upper_lim]
        if sat.sat_type == 'tree':
            sat.tree_mask = self.mask_tree(sat)
        elif sat.sat_type == 'tree.lg':
            sat.tree_mask = self.mask_tree_lg(sat)
        elif sat.sat_type == 'hal':
            sat.catalog_mask = self.multiple_catalog_masks(sat, mask_names=['star.mass'])
        elif sat.sat_type == 'hal.lg':
            sat.catalog_mask = self.mask_lg_baryon_cat(sat)
...
class IsotropicHalo():
    def hal_iso(self, sat, mask_names, sat_data=None):
    ...
    def hal_iso_lite(self, sat_data, sat_params, mask_names):
        iso_mask_dict = defaultdict(list)
    ...
    def tree_iso(self, sat, mask_names):
        """
...
# this should probably inherit from CatalogParam
class LoopHalo(object):
    def __init__(self):
        pass

    def loop_sat(self, sat, exec_func, **kwargs):
        '''
    ...
    def loop_hal(self, sat, mask_key, exec_func, **kwargs):
        '''
    ...
    def loop_hal_lite(self, sat_data, sat_param, mask_key, exec_func, **kwargs):
        '''
        Loop a function over a series of halo catalogs for each simulation in a sat 
        object.
        ...
        '''
        loop_dict = defaultdict(list)
        if sat_param.sat_type == 'hal.lite':
            for host_key in sat_param.hal_name:
                for redshift_index in range(len(sat_param.redshift)):
                    hal = sat_data[host_key][redshift_index]
                    hal_mask = sat_param.catalog_mask[host_key][redshift_index][mask_key]
                    loop_dict[host_key].append(exec_func(hal=hal, hal_mask=hal_mask, **kwargs))
        elif sat_param.sat_type == 'hal.lg.lite':
            for pair_name in sat_data.keys():
                for host_name, host_str in zip(sat_param.hal_name[pair_name], ['host.', 'host2.']):
                    for redshift_index in range(len(sat_param.redshift)):
                        cat = sat_data[pair_name][redshift_index]
                        cat_mask = sat_param.catalog_mask[pair_name][host_name][redshift_index][mask_key]
                        loop_dict[host_name].append(exec_func(hal=cat, hal_mask=cat_mask, host_str=host_str, **kwargs))
        else:
    ...
    def loop_iso(self, sat, mask_key, exec_func, **kwargs):
        '''
    ...
    def loop_iso_lite(self, sat_params, mask_key, exec_func, **kwargs):
        '''
        Loop over a time series of isotropic satellite distributions.
        '''
        loop_dict = defaultdict(list)
    ...
    # doesn't work
    def loop_iso_parallel(self, sat, mask_key, exec_func, kwargs):
        '''
        Loop over a time series of isotropic satellite distributions.
        '''
    ...
    def iso_snapshot_loop(self, exec_func, tree, host_masks, mask_key, snapshot_index, kwargs):
        px = exec_func(
            hal=tree, 
    ...
    # doesn't work
    def loop_hal_parallel(self, sat_data, sat_param, mask_key, exec_func, **kwargs):
        import multiprocessing as mp

        loop_dict = defaultdict(list)
        if sat_param.sat_type == 'tree':
            for host_name in sat_data.tree.keys():
                pool = mp.Pool()
                loop_dict[host_name] = [pool.apply(self.snapshot_loop, 
                    args=(exec_func, sat_data[host_name], 
                    sat_param.tree_mask[host_name], mask_key, snapshot_ind, 
                    kwargs)) for snapshot_ind in range(len(sat_param.redshift))]
                pool.close()
                pool.join()
                
        elif sat_param.sat_type == 'tree.lg':
            for pair_name in sat_data.tree.keys():
                for host_name, host_str in zip(sat_param.hal_name[pair_name], ['host.', 'host2.']):
                    loop_dict[host_name] = [pool.apply(self.snapshot_loop_lg, 
                        args=(exec_func, sat_data, host_name, mask_key, 
                        pair_name, host_str, snapshot_ind), **kwargs) for 
                        snapshot_ind in range(len(sat_param.redshift))]

        elif sat_param.sat_type == 'hal.lg.lite':
            for pair_name in sat_data.hal_catalog.keys():
                for host_name, host_str in zip(sat_param.hal_name[pair_name], ['host.', 'host2.']):
                    for redshift_index in range(len(sat_param.redshift)):
                        cat = sat_data[pair_name][redshift_index]
                        cat_mask = sat_param.catalog_mask[pair_name][host_name][redshift_index][mask_key]
                        loop_dict[host_name].append(exec_func(hal=cat, 
                            hal_mask=cat_mask, host_str=host_str, **kwargs))
        
        elif sat_param.sat_type == 'hal.lite':
            for hal_name in sat_data.keys():
                pool = mp.Pool()
                loop_dict[hal_name] = [pool.apply(self.snapshot_loop_hal, 
                    args=(exec_func, 
                    sat_data[hal_name][snapshot_ind], 
                    sat_param.catalog_mask[hal_name][snapshot_ind][mask_key], 
                    kwargs)) for snapshot_ind in range(len(sat_param.redshift))]
                pool.close()
                pool.join()
        '''
        elif sat_param.sat_type == 'hal.lite':
            pool = mp.Pool(len(sat_data.keys()))
            loop_dict = [pool.apply(self.host_loop_hal, 
                args=(exec_func, 
                sat_data[hal_name], 
                sat_param.catalog_mask[hal_name], 
                len(sat_param.redshift),
                mask_key,
                kwargs)) for hal_name in sat_data.keys()]
            pool.close()
            pool.join()
        '''
    ...
    def snapshot_loop(self, exec_func, tree, host_masks, mask_key, snapshot_index, kwargs):
        px = exec_func(
            hal=tree, 
            hal_mask=host_masks[snapshot_index][mask_key], 
            **kwargs)
    ...
    def snapshot_loop_hal(self, exec_func, hal, host_masks, kwargs):
        px = exec_func(
            hal=hal, 
            hal_mask=host_masks, 
            **kwargs)
    ...
    def snapshot_loop_lg(self, exec_func, sat, host_name, mask_key, pair_name, host_str, snapshot_index, kwargs):
        px = exec_func(
    ...
    def host_loop_hal(
        self, exec_func, hal, host_masks, redshift_len, mask_key, kwargs):
        px = []
        for z_index in range(redshift_len):
            px.append(exec_func(hal=hal[z_index], 
                hal_mask=host_masks[z_index][mask_key], **kwargs))
...
def halo_track(tree, initial_tree_ids, redshift_list=None, snapshot_list=None):
    ...
    hal_tracker = np.empty((len(snapshot_ids), len(initial_tree_ids)))
    ...
    while i >= np.min(snapshot_ids):
        i -= 1
        progenitor_indices = tree['progenitor.main.index'][tracking_ids.astype(int)]
        if i in snapshot_ids:
            tracking_ids = optim_track(tracking_ids, progenitor_indices)
            hal_tracker[i] = tracking_ids.astype(int)
